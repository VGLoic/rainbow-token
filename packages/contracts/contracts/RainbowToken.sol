...
contract RainbowToken {
  ...
  function blend(
    ...
    blenderPlayer.color.r = uint8(
      (uint16(blenderPlayer.color.r) + uint16(otherPlayer.color.r)) / 2
    );
    blenderPlayer.color.g = uint8(
      (uint16(blenderPlayer.color.g) + uint16(otherPlayer.color.g)) / 2
    );
    blenderPlayer.color.b = uint8(
      (uint16(blenderPlayer.color.b) + uint16(otherPlayer.color.b)) / 2
    );

    emit Blended(
      msg.sender,
      blenderPlayer.color.r,
      blenderPlayer.color.g,
      blenderPlayer.color.b
    );
  }

  function selfBlend() external payable gameInProgress onlyPlayer {
    require(
      msg.value >= SELF_BLEND_PRICE,
      "Not enough sent Ethers to self blend"
    );
    Player storage blenderPlayer = players[msg.sender];
    blenderPlayer.color.r = uint8(
      (uint16(blenderPlayer.color.r) + uint16(blenderPlayer.defaultColor.r)) / 2
    );
    blenderPlayer.color.g = uint8(
      (uint16(blenderPlayer.color.g) + uint16(blenderPlayer.defaultColor.g)) / 2
    );
    blenderPlayer.color.b = uint8(
      (uint16(blenderPlayer.color.b) + uint16(blenderPlayer.defaultColor.b)) / 2
    );
  ...
  function setBlendingPrice(uint256 newBlendingPrice)
    external
    gameInProgress
    onlyPlayer
  {
  ...
  function claimVictory(address winnerAddress)
    external
    onlyManager
    gameInProgress
  {
    ...
    require(
      (player.color.r - target.r) *
        (player.color.r - target.r) +
        (player.color.g - target.g) *
        (player.color.g - target.g) +
        (player.color.b - target.b) *
        (player.color.b - target.b) <=
        25,
  ...
  function getPlayers(address[] memory playerAddresses)
    external
    view
    returns (Player[] memory)
  {
